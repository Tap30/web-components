  ...
  paths() {
    ...
    return manifest.modules
      ...
      .map(module => {
        ...
        if (!["tap-modal", "tap-pinwheel-group"].includes(component.tagName))
          content += `<custom-element-manifest-viewer theme="github-dark-default" tag-name=${component.tagName} config="./elements-config.json" manifest="./custom-elements.json"></custom-element-manifest-viewer>\n`;
        ...
        if (!!component.members?.length) {
          ...
          (component.members as PropertyLike[]).forEach(member => {
            content += `| \`${member.name}\` | ${member.type?.text
              ?.split("|")
              .map(t => `\`${t.trim()}\``)
              .join(
                " \\| ",
              )} | \`${member.default}\` | ${member.description} |\n`;
          });
        }
...
const getPageTitle = (componentName: string): string => {
  const result = componentName?.replace(/^-*(.)|-+(.)/g, (_, c, d) =>
