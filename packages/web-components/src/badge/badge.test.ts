...
describe("🧩 badge", () => {
  ...
  test("🧪 should show value when variant is not `dot`", async ({ page }) => {
    ...
    await render(
      page,
      `<tapsi-badge data-testid="test-badge" variant="dot" value="20"></tapsi-badge>`,
    );
    await expect(badge.getByText("20")).toBeHidden();

    await render(
      page,
      `<tapsi-badge data-testid="test-badge" variant="numeral" value="20"></tapsi-badge>`,
    );
    await expect(badge.getByText("20")).toBeVisible();

    await render(
      page,
      `<tapsi-badge data-testid="test-badge" variant="pill" value="20"></tapsi-badge>`,
    );
    await expect(badge.getByText("20")).toBeVisible();
  });

  test("🧪 should render slots when variant is `pill`", async ({ page }) => {
    const iconSlot = `
    ...
    const icon = page.getByTestId("test-badge-icon-slot");

    await render(
      page,
      `
      <tapsi-badge data-testid="test-badge" variant="dot" value="20">${iconSlot}</tapsi-badge>`,
    );
    ...
    await render(
      page,
      `
      <tapsi-badge data-testid="test-badge" variant="numeral" value="20">${iconSlot}</tapsi-badge>`,
    );
    await expect(icon).toBeHidden();
    ...
    await render(
      page,
      `
      <tapsi-badge data-testid="test-badge" variant="pill" value="20">${iconSlot}</tapsi-badge>`,
    );
