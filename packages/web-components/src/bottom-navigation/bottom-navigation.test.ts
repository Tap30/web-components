...
const expectBottomNavigationItems = (chips: Locator[]) => {
  return {
    toBeActive: async () => {
      for (const chip of chips) {
        await expect(chip).toHaveAttribute("active");
      }
    },
    notToBeActive: async () => {
      for (const chip of chips) {
        await expect(chip).not.toHaveAttribute("active");
      }
    },
  };
};
...
describe("🧩 bottom-navigation", () => {
  ...
  test("🧪 should trigger `activate` event for toggling each item", async ({
    ...
    await expectBottomNavigationItems([item1]).toBeActive();
    await expectBottomNavigationItems([item2]).notToBeActive();

    await item2.click();
    await handleActivate1.matchResult({ callCount: 1 });
    await handleActivate2.matchResult({ callCount: 1 });
    await expectBottomNavigationItems([item2]).toBeActive();
    await expectBottomNavigationItems([item1]).notToBeActive();
  ...
  test("🧪 should trigger the `activechange` event by changing the active item using click", async ({
    ...
    await expectBottomNavigationItems([item1]).toBeActive();
    await expectBottomNavigationItems([item2, item3, item4]).notToBeActive();

    await item2.click();
    await fn.matchResult({ callCount: 2 });
    await expectBottomNavigationItems([item2]).toBeActive();
    await expectBottomNavigationItems([item1, item3, item4]).notToBeActive();

    await item3.click();
    await fn.matchResult({ callCount: 3 });
    await expectBottomNavigationItems([item3]).toBeActive();
    await expectBottomNavigationItems([item1, item2, item4]).notToBeActive();

    await item4.click();
    await fn.matchResult({ callCount: 4 });
    await expectBottomNavigationItems([item4]).toBeActive();
    await expectBottomNavigationItems([item1, item2, item3]).notToBeActive();
  });
