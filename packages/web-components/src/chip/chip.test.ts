...
describe("🧩 chip", () => {
  ...
  test("🧪 should toggle with keyboard interaction", async ({ page }) => {
    await render(
      page,
      `<tapsi-chip data-testid="test-chip" value="value">چیپ</tapsi-chip>`,
    );
...
<tapsi-chip data-testid="test-chip" value="value">
    ...
    const leadingSlot = page.getByTestId("test-chip-leading-slot");
    const trailingSlot = page.getByTestId("test-chip-trailing-slot");

    await expect(leadingSlot).toBeVisible();
    await expect(trailingSlot).toBeVisible();
  });
});
