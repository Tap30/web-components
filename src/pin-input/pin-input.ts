...
export class PinInput extends LitElement {
    ...
    @property({type: Boolean, attribute: 'auto-focus-first-cell'}) autoFocusFirstCell: boolean = true;
    ...
    private handleOverflowedCell(event: CustomEvent<ValueChangedEventParams>) {
        let overflowedText = event.detail.value;
        const cellIndex = event.detail.index;
        const isLongerThanRemainingCells = overflowedText.length > this.lastCellIndex - cellIndex;

        if (isLongerThanRemainingCells) {
            overflowedText = overflowedText.slice(0, this.lastCellIndex - cellIndex)
        }

        this.fillCells(overflowedText, cellIndex + 1)
    }
    ...
    get cellValues() {
        return [...this._cells].map((_cell, index) => _cell.value)
    }
                            ...
                            (_, index) => {
                                return html`
                                    <tap-pin-input-cell
                                            ...
                                            @cell-filled=${this.handleCellFilled}
                                            @cell-cleared=${this.handleCellCleared}
                                            @overflow-value=${this.handleOverflowedCell}
